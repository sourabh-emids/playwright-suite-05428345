...
def unique_title_meta_description(page: Page) -> None:
    ...
    title = page.title()
    assert title is not None and title.strip() != "", "Page should have a title"
    assert len(title) > 10, "Title should be meaningful"
    
    meta_desc = page.locator('meta[name="description"]').get_attribute("content")
    assert meta_desc is not None and meta_desc.strip() != "", "Page should have meta description"
...
def canonical_url_set(page: Page) -> None:
    ...
    assert canonical is not None, "Canonical URL should be set"
    assert "emids.com" in canonical, "Canonical should be on emids.com domain"
...
def social_preview_metadata(page: Page) -> None:
    ...
    assert og_title is not None or og_description is not None or og_image is not None, "OG metadata should be configured"
